package com.priceGrid.priceGrid;

import com.priceGrid.priceGrid.controller.PriceGridController;
...
@SpringBootTest
public class PriceGridServiceTest {
    @Mock
    private PriceGridService service;

    @InjectMocks
    private PriceGridController priceGridController;

    private MockMvc mockMvc;

    @BeforeEach
    void setup(){
        mockMvc = MockMvcBuilders.standaloneSetup(priceGridController).build();
    }

    @Test
    void getPricesPositiveTestCase() throws Exception{
        when(service.getPrices()).thenReturn(getTableData());
        mockMvc.perform(get("/api/prices"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.widths").exists())
                .andExpect(jsonPath("$.heightModels").exists());
    }

    @Test
    void getPricesNegativeTestCase() throws Exception{
        when(service.getPrices()).thenReturn(null);
        mockMvc.perform(get("/api/prices"))
                .andExpect(status().isNoContent())
                .andExpect(jsonPath("$.widths").doesNotExist())
                .andExpect(jsonPath("$.heightModels").doesNotExist());
    }
